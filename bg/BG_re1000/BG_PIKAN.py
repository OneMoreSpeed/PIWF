...
layers_hidden= [2,50,50,50,50,50,50,1]
...
for ep in tqdm(range(num_epochs)):
       ...
       #loss_f = torch.mean(utils.ac_equation(u_hat_f, variables_f) ** 2)
        ...
        log_Name = "N{}_lr{:.1e}".format(layers_hidden, lr)
        if ep % 10000 == 0:
...
np.save(f'{layers_hidden}layer_LBFGS_train_loss.npy', results['train_loss'])
np.save(f'{layers_hidden}layer_LBFGS_pde_loss.npy', results['pde_loss'])
np.save(f'{layers_hidden}layer_LBFGS_test_loss.npy',results['test_loss'])
np.save(f'{layers_hidden}layer_LBFGS_bc_loss.npy',results['bc_loss'])
