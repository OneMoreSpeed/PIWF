...
if __name__ == "__main__":
    ...
    for n in range(num_T):
        ...
        # 将NumPy数组保存到文件中
        np.save(f'u_predNS_T{aim_T[n]}_{num_layers}.npy', pred_u)
        np.save(f'u_resNS_T{aim_T[n]}_{num_layers}.npy', res_u)
        np.save(f'v_predNS_T{aim_T[n]}_{num_layers}.npy', pred_v)
        np.save(f'v_resNS_T{aim_T[n]}_{num_layers}.npy', res_v)
        np.save(f'p_predNS_T{aim_T[n]}_{num_layers}.npy', pred_p)
        np.save(f'p_resNS_T{aim_T[n]}_{num_layers}.npy', res_p)
