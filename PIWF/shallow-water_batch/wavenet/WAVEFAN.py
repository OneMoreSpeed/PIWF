...
class Net(nn.Module):
    def __init__(self):
        ...
        nn.init.xavier_normal_(self.conv1.weight, gain=nn.init.calculate_gain('tanh'))
        self.bn1 = nn.BatchNorm1d(5)
        self.conv2 = nn.Conv1d(1, 5, 3, 1)
        nn.init.xavier_normal_(self.conv2.weight, gain=nn.init.calculate_gain('tanh'))
        self.bn2 = nn.BatchNorm1d(5)

        self.conv11 = nn.Conv1d(5, 20, 2, 1)
        nn.init.xavier_normal_(self.conv11.weight, gain=nn.init.calculate_gain('tanh'))
        self.bn11 = nn.BatchNorm1d(20)
        self.conv22 = nn.Conv1d(5, 20, 2, 1)
        nn.init.xavier_normal_(self.conv22.weight, gain=nn.init.calculate_gain('tanh'))
        self.bn22 = nn.BatchNorm1d(20)

        self.hidden1 = nn.Linear(1, 50)
        torch.nn.init.xavier_normal_(self.hidden1.weight, gain=1)
        self.hidden2 = nn.Linear(1, 50)
        torch.nn.init.xavier_uniform_(self.hidden2.weight, gain=1)

        #self.hidden3 = nn.Linear(3, 50)
        #torch.nn.init.xavier_normal_(self.hidden2.weight, gain=1.0)
        self.hid1 = nn.Linear(50,1)
        torch.nn.init.xavier_normal_(self.hid1.weight, gain=1)
        self.hid2 = nn.Linear(50,1)
        torch.nn.init.xavier_uniform_(self.hid2.weight, gain=1)
    ...
    def forward(self, data):
        data1 = F.tanh(self.bn1(self.conv1(data)))
        data2 = F.tanh(self.bn2(self.conv2(data)))
        #data3 = F.tanh(self.bn3(self.conv3(data)))

        data1 = F.tanh(self.bn11(self.conv11(data1)))
        data2 = F.tanh(self.bn22(self.conv22(data2)))
        #data3 = F.tanh(self.bn33(self.conv33(data3)))
        #data1 = data.view(-1, self.num_flat_features(data))
        data1 = self.hidden1(data1)
        data2 = self.hidden2(data2)
        #data3 = self.hidden3(data3)
        h  = self.hid1(data1)
        u  = self.hid2(data2)
        #v  = self.hid3(data3)
        #h  = torch.abs(h)
...
class l_channelattention(nn.Module):
    ...
    def __init__(self,
        ...
        self.MLPHead = MLPHead(channel_size=50,hidden_size=256,out_size=50)
...
class WAVEFANBLOCK(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, hidden_dim=2048, num_layers=3):
        ...
        self.l_channelattention = l_channelattention(hidden_dim=50)
...
class WAVEFAN(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, hidden_dim=2048, num_layers=3):
        ...
        self.embedding = nn.Linear(input_dim, hidden_dim)
        self.l_channelattention = l_channelattention(hidden_dim=hidden_dim)
        self.layers = nn.ModuleList()
        for _ in range(num_layers - 1):
