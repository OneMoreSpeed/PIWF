...
def plot_zoomed_subplots():
    ...
    zoom_configs = [
        (900, [200, 400], [-0.05, 0.15], "zoom_t900"),
        (1800, [450, 650], [-0.05, 0.15], "zoom_t1800"),
        (2700, [750, 850], [-0.05, 0.15], "zoom_t2700")
    ]
    ...
    for t_point, x_range, y_range, filename_prefix in zoom_configs:
        ...
        # 确保刻度包含两端点
        x_major_ticks = []
        current_tick = x_range[0]
        while current_tick <= x_range[1] + 0.001:
            x_major_ticks.append(current_tick)
            current_tick += x_major_step

        # 如果最后一个刻度超过了范围，调整一下
        if x_major_ticks[-1] > x_range[1]:
            # 如果最后一个刻度离右端点较近，保留它
            if x_major_ticks[-1] - x_range[1] < x_major_step * 0.3:
                x_major_ticks[-1] = x_range[1]
            else:
                x_major_ticks = x_major_ticks[:-1]

        # 确保包含右端点
        if len(x_major_ticks) > 0 and abs(x_major_ticks[-1] - x_range[1]) > 0.001:
        ...
        # 确保刻度包含两端点
        y_major_ticks = []
        current_tick = y_range[0]
        while current_tick <= y_range[1] + 0.0001:
            y_major_ticks.append(current_tick)
            current_tick += y_major_step

        # 如果最后一个刻度超过了范围，调整一下
        if y_major_ticks[-1] > y_range[1]:
            # 如果最后一个刻度离上端点较近，保留它
            if y_major_ticks[-1] - y_range[1] < y_major_step * 0.3:
                y_major_ticks[-1] = y_range[1]
            else:
                y_major_ticks = y_major_ticks[:-1]

        # 确保包含上端点
        if len(y_major_ticks) > 0 and abs(y_major_ticks[-1] - y_range[1]) > 0.0001:
        ...
        for tick in y_major_ticks:
            if abs(tick) < 0.0001:  # 接近0
                y_tick_labels.append("0")
            elif abs(tick) < 0.01:  # 很小的数
                label = f"{tick:.3f}"
                # 移除尾随的零
                if '.' in label:
                    label = label.rstrip('0').rstrip('.')
                y_tick_labels.append(label)
            elif abs(tick) < 0.1:  # 较小的数
                label = f"{tick:.2f}"
                if '.' in label:
                    label = label.rstrip('0').rstrip('.')
                y_tick_labels.append(label)
