...
def generate_inputPNT(
    ...
    -----
        ranges : list or np.array; shape (2,) or (n_var, 2)
            the range of input variables. Default: [-1,1].
        pnt_num : int
            the number of point samples. Default: 1000.
        normalize_input : bool
            If True, apply normalization to inputs. Default: False.
        normalize_label : bool
            If True, apply normalization to labels. Default: False.
    ...
    Example
    -------
    >>> pde_input = generate_inputPNT(n_var=2, pnt_num=100)
    >>> pde_input.shape
    torch.Size([100, 2])
    '''
    ...
    sample_input = torch.zeros(pnt_num, n_var)
    for i in range(n_var):
        if n_var == 3 and i == 2:  # 当 n_var=3 且为第三列时
            # 创建从 ranges[i,0] 到 ranges[i,1] 的数组，步长为 0.1
            step = 0.1
            all_values = torch.arange(ranges[i, 0], ranges[i, 1] + step, step)
            
            # 确保生成的array的长度足够，如果不到pnt_num,则重复
            if len(all_values) < pnt_num:
                num_per_value = pnt_num // len(all_values)
                ind = torch.arange(len(all_values))
                indices = ind.repeat(num_per_value)
                
            else:
                # 选择 pnt_num 个等间隔的值
                indices = torch.linspace(0, len(all_values) - 1, pnt_num).long()

            sample_input[:, i] = all_values[indices]
        else:
            sample_input[:,i] = torch.rand(pnt_num,)*(ranges[i,1]-ranges[i,0])+ranges[i,0]
...
def fit_params(x, y, fun, a_range=(-10,10), b_range=(-10,10), grid_number=101, iteration=3, verbose=True, device='cpu'):
    ...
    for _ in range(iteration):
        a_ = torch.linspace(a_range[0], a_range[1], steps=grid_number).to(device)
        b_ = torch.linspace(b_range[0], b_range[1], steps=grid_number).to(device)
