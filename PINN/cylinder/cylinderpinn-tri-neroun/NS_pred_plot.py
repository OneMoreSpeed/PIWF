...
if __name__ == "__main__":
    ...
    #指定时间点
    aim_T = np.array([0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7])#这里对应的是取0,1,2,3,4,5,6,7s的数据
    num_T = len(aim_T)
    num_hidden = 4
    num_nodes = 384
    ...
    checkpoint = torch.load('./N[3, 384, 384, 384, 384, 3]_lr1.0e-03/weight100000.checkpoint', map_location=device)
    ...
    for n in range(num_T):
        ...
        res_u_mean = np.mean(res_u)
        res_v_mean = np.mean(res_v)
        res_p_mean = np.mean(res_p)
        res_u_mean_list.append(res_u_mean)
        res_v_mean_list.append(res_v_mean)
        res_p_mean_list.append(res_p_mean)
        ...
        # 将NumPy数组保存到文件中
    np.save('res_u_mean_PINN.npy', res_u_mean_list)
    np.save('res_v_mean_PINN.npy', res_v_mean_list)
    np.save('res_p_mean_PINN.npy', res_p_mean_list)
