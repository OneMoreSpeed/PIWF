...
if __name__ == "__main__":  
    ...
    fig_Path = os.path.join(cwd, "figure_pred")
    ...
    for aaa in ['h', 'hu']:
        ...
        u_pred = np.load(f"swe_stable_{aaa}.npy")
        ...
        if aaa == 'h':
            ...
            var_min = 0 #[0,-1e-6,-1e-6]
            var_max = 0.5#[0.5,1e-6,1e-6]
            var_inc = 0.1#[0.1,5e-7,5e-7]
            cb_label = r"$h$"

        if aaa == 'hu':
            var_min = 0  # [0,-1e-6,-1e-6]
            var_max = 0.15  # [0.5,1e-6,1e-6]
            var_inc = 0.03  # [0.1,5e-7,5e-7]
            cb_label = r"$hu$"
        ...
        #, r"$\mathdefault{\bar U_y}$", r"$\mathdefault{\bar U_z}$"]
        ...
        cb.ax.text(s=cb_label, x=-0.22, y=0.25, va="center", ha="left", transform=cb.ax.transAxes, fontsize=cb_FontSize)
